...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        # Update this line to filter only valid image files
        self.instance_images_path = [
            p for p in Path(instance_data_root).iterdir() if p.suffix.lower() in VALID_IMAGE_EXTENSIONS
        ]
        # had to fix it to remove files generated by azure such as .amlignore
        ...
        if class_data_root is not None:
            ...
            self.class_images_path = [
                p for p in Path(class_data_root).iterdir() if p.suffix.lower() in VALID_IMAGE_EXTENSIONS
            ]
